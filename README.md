# Velmie Wallet Notifications service
...
|----------------------------------|----------|------------------------------------|---------------------|
| VELMIE_WALLET_NOTIFICATIONS_SERVER_PORT  | yes      | Port number to start API server()  | 10050               |
| VELMIE_WALLET_NOTIFICATIONS_RPC_PORT     | yes      | Port number to start RPC server    | RPCPort             |
| VELMIE_WALLET_NOTIFICATIONS_DB_HOST      | yes      | Database host                      | localhost           |
| VELMIE_WALLET_NOTIFICATIONS_DB_PORT      | yes      | Database port                      | 3306                |
| VELMIE_WALLET_NOTIFICATIONS_DB_NAME    | yes      | Database schema name               | notifications |
| VELMIE_WALLET_NOTIFICATIONS_DB_USER      | yes      | Database user                      | root                |
| VELMIE_WALLET_NOTIFICATIONS_DB_PASSWORD  | yes      | Database password                  | secret              |
| VELMIE_WALLET_NOTIFICATIONS_PUSH_TOKEN_TTL_DAYS  | no | We define how much days the system consider push tokens as not expired. The system use this value to send push notification only to not expired push tokens. | 14 |

**Initial data**  

See [initial data](docs/initial_data.md)
## Wallet Notifications Helm chart configuration
...
The following table lists the configurable parameters of the wallet-notifications chart, and their default values.
...
| appApiPathPrefix               | API prefix path. Used with internal health check functionality.                                                                  | notifications                              |
...
| nameOverride                   | Override this chart name.                                                                                                        | wallet-notifications                        |
